...
import { MODEL_CATALOG, CATEGORY_LABELS, FAMILY_LABELS, type ModelFamily } from "../../lib/modelCatalog";
import { SectionHeader } from "../ui/SectionHeader";
import { HardDrive, Layers, Box, Star, ArrowRight } from "lucide-react";

/** Logo image path for each model family */
const FAMILY_LOGO: Record<ModelFamily, string | null> = {
  llama: "/logos/meta-llama.png",
  qwen: "/logos/qwen.png",
  phi: "/logos/microsoft.png",
  gemma: "/logos/google.png",
  mistral: "/logos/mistral.png",
  deepseek: "/logos/deepseek.png",
  smollm: "/logos/huggingface.png",
  olmo: "/logos/allenai.png",
  stablelm: "/logos/stability.png",
  redpajama: "/logos/togetherai.png",
  tinyllama: "/logos/meta-llama.png",
  ministral: "/logos/mistral.png",
  hermes: "/logos/nousresearch.png",
  other: null,
};

/** Hand-picked showcase: one best model per family to show all logos */
const SHOWCASE_IDS = [
  "Llama-3.2-3B-Instruct-q4f32_1-MLC",        // Llama (Meta)
  "Qwen3.5-2B-q4f16_1-MLC",                    // Qwen (Alibaba)
  "Phi-4-mini-instruct-q4f16_1-MLC",            // Phi (Microsoft)
  "gemma3-1b-it-q4f16_1-MLC",                   // Gemma (Google)
  "Mistral-7B-Instruct-v0.3-q4f16_1-MLC",       // Mistral
  "DeepSeek-R1-Distill-Qwen-7B-q4f16_1-MLC",    // DeepSeek
  "SmolLM2-1.7B-Instruct-q4f16_1-MLC",          // SmolLM (HuggingFace)
  "Hermes-3-Llama-3.2-3B-q4f16_1-MLC",          // Hermes (NousResearch)
  "stablelm-2-zephyr-1_6b-q4f16_1-MLC",         // StableLM (Stability)
];
...
export function ModelsSection() {
  ...
  const showcase = SHOWCASE_IDS
    ...
    .filter(Boolean) as (typeof MODEL_CATALOG)[number][];
  ...
  return (
    <section id="models" className="py-24 sm:py-32 px-6">
      <div className="max-w-6xl mx-auto">
        ...
        >
          {showcase.map((model) => {
            const logo = FAMILY_LOGO[model.family];
            return (
              <motion.div
                key={model.id}
                variants={fadeUp}
              ...
              >
                <div className="flex items-start gap-3 mb-3">
                  {logo ? (
                    <img
                      src={logo}
                      alt={FAMILY_LABELS[model.family]}
                      className="w-10 h-10 rounded-xl object-cover shrink-0"
                    />
                  ) : (
                    <div className="w-10 h-10 rounded-xl bg-gray-500/10 flex items-center justify-center shrink-0 text-xs font-bold text-gray-400">
                      ?
                    </div>
                  )}
